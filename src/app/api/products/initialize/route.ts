...
export async function POST() {
    try {
        console.log("🚀 Initializing Comprehensive Hardware Catalog...");
            ...
            if (!existing) {
                ...
                await prisma.product.create({
                    data: {
                        ...item,
                        sku,
                        stock: 0 // Zero stock until added via Stock Inward
                    }
                });
                createdCount++;
            } else {
        ...
        return NextResponse.json({
            message: "Hardware Catalog Ready",
            details: {
                created: createdCount,
                skipped: skippedCount,
                total: HARDWARE_ITEMS.length
            }
        });
    } catch (error: any) {
